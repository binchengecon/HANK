...
F_init = -W1_mat**2
F_init_comp = F_init[1:-2,:,:]
# F_init = -W1_mat**2

a_star = np.zeros(W1_mat.shape)
c_star = np.zeros(W1_mat.shape)

a_star_comp = a_star[1:-2,:,:]
c_star_comp = c_star[1:-2,:,:]

dVec = np.array([hW1, hW2, hW3])
increVec = np.array([1, nW1, nW1*nW2], dtype=np.int32)
increVec_comp = np.array([1, (nW1-2), (nW1-2)*nW2], dtype=np.int32)

petsc_mat = PETSc.Mat().create()
petsc_mat.setType('aij')
petsc_mat.setSizes([(nW1-2) * nW2 * nW3, (nW1-2) * nW2 * nW3])
